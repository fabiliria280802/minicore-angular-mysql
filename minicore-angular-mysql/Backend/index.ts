...
import { Contrat } from "../Backend/Models/Contrat";
...
const getReport = async (req: Request, res: Response) => {
    ...
    const report: { name: string; amountTotal: number }[] = [];
    ...
    const contratsLista: any[] = (await Contrat.findAll({ raw: true }).then()).filter(
        (contrat: any) => {
            const fecha = new Date(contrat.fecha);
    ...
    clientsLista.forEach((client: any) => {
        ...
        report.push({ name: client.nombre, amountTotal });
    });
